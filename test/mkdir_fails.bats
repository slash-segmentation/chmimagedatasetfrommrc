...
@test "Test where mkdir fails" {
  ...
  echo "hi=1" > "$THE_TMP/data"
  # Run kepler.sh
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -CWS_jobname jname -CWS_user joe -CWS_jobid 123 -CWS_outputdir $THE_TMP -mrc "$THE_TMP/input" -imodSourceScript "$THE_TMP/data" $WF
  ...
  [ "${lines[0]}" == "simple.error.message=Error running mkdir" ]
  [[ "${lines[1]}" == "detailed.error.message=Non zero exit code from mkdir -p $THE_TMP/data : mkdir:"* ]]
